    ...
    def _telnet_process_subnegotiation(self, suboption):
        ...
        if suboption[0:1] == COM_PORT_OPTION:
            ...
            if suboption[1:2] == SET_BAUDRATE:
                backup = self.serial.boudrate
                try:
                    (baudrate,) = struct.unpack(b"!I", suboption[2:6])
                    if baudrate != 0:
                        self.serial.boudrate = baudrate
                except ValueError as e:
                    if self.logger:
                        self.logger.error("failed to set baud rate: {}".format(e))
                    self.serial.boudrate = backup
                else:
                    if self.logger:
                        self.logger.info("{} baud rate: {}".format('set' if baudrate else 'get', self.serial.boudrate))
                self.rfc2217_send_subnegotiation(SERVER_SET_BAUDRATE, struct.pack(b"!I", self.serial.boudrate))
